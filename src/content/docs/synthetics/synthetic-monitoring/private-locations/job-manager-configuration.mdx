...
Where $D_{avg,m}$ is your **average job execution duration in seconds** (from the first NRQL query).
...
You can simplify maintenance by treating each SJM deployment as a fixed-capacity unit.

1. **Set Fixed Values:** For *each* SJM `values.yaml` file, set `parallelism` to a reasonable maximum value at or near your $P_{max}$ (from [Step 2](#step-2-check-against-the-single-sjm-throughput-limit)). This allows for fluctuations in average job duration.
2. **Set Completions:** Set the `completions` value for each deployment to 6-10x your fixed `parallelism` value (e.g., `parallelism: 20`, `completions: 200`). Use the completions formula to get an [estimate based on your average job duration](#estimate-completions-and-parallelism).
3. **Install Releases:** Install as many separate Helm releases as you need to handle your total $P_{req}$ (from [Step 1](#step-1-estimate-your-required-workload)). For example, if your total $P_{req}$ is 60 and you've fixed each SJM's `parallelism` at 20 ($P_{max}$ from [Step 2](#step-2-check-against-the-single-sjm-throughput-limit)), you would need **three** separate Helm deployments to meet the required job demand.
